...
TARGET = SolARServiceTest_Mapping_Multi_Relocalization
VERSION=1.0.0
...
SOURCES += \
    SolARServiceTest_Mapping_Multi_Relocalization.cpp
...
DISTFILES += \
    SolARServiceTest_Mapping_Multi_Relocalization_conf.xml \
    packagedependencies.txt \
    docker/build.sh \
    docker/launch.bat \
    docker/launch.sh \
    docker/launch_vm.sh \
    docker/SolARServiceMappingMultiRelocalizationClient.dockerfile \
    docker/start_client.sh

xml_files.path = $${TARGETDEPLOYDIR}
xml_files.files =  $$files($${PWD}/SolARServiceTest_Mapping_Multi_Relocalization_conf.xml)
